...
public class ParametrosResource {
   ...
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(value = "/parametros/{id}", consumes = "application/merge-patch+json")
  public ResponseEntity<Parametros> partialUpdateParametros(
    ...
    Optional<Parametros> result = parametrosService.partialUpdate(parametros);

    return ResponseUtil.wrapOrNotFound(
      result,
      HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, parametros.getId().toString())
    );
  }
