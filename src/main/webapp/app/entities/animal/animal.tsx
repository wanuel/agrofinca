						...
						<tbody>
							{animalList.map((animal, i) => (
								<tr key={`entity-${i}`}>
									...
									<td>{animal.tipo ? <Link to={`parametros/${animal.tipo.id}`}>{animal.tipo.descripcion}</Link> : ''}</td>
									<td>{animal.raza ? <Link to={`parametros/${animal.raza.id}`}>{animal.raza.descripcion}</Link> : ''}</td>
									<td className="text-right">
										<div className="btn-group flex-btn-group-container">
											...
											<Button tag={Link} to={`${match.url}/${animal.id}`} color="info" size="sm">
												...
												<span className="d-none d-md-inline">Ficha</span>
											</Button>
										</div>
									</td>
								</tr>
