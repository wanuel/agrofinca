...
export interface IAnimalVacunasProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const AnimalVacunas = (props: IAnimalVacunasProps) => {
  ...
  const { animalVacunasList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="animal-vacunas-heading">
            ...
            <tbody>
              {animalVacunasList.map((animalVacunas, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{animalVacunas.animal ? <Link to={`animal/${animalVacunas.animal.id}`}>{animalVacunas.animal.nombre}</Link> : ''}</td>
                  <td>{animalVacunas.tipo ? <Link to={`parametros/${animalVacunas.tipo.id}`}>{animalVacunas.tipo.descripcion}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${animalVacunas.id}`} color="info" size="sm">
...
const mapStateToProps = ({ animalVacunas }: IRootState) => ({
  animalVacunasList: animalVacunas.entities,
  loading: animalVacunas.loading,
  totalItems: animalVacunas.totalItems,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(AnimalVacunas);
