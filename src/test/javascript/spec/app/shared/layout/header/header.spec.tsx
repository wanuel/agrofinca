...
import { AdminMenu, FincaMenu, AnimalesMenu, ContabilidadMenu, InventariosMenu, ConfiguracionMenu, AccountMenu, LocaleMenu } from 'app/shared/layout/menus';
...
describe('Header', () => {
  ...
  it('Renders a Header component in dev profile with LoadingBar, Navbar, Nav and dev ribbon.', () => {
    ...
    expect(nav.find(FincaMenu).length).toEqual(1);
	expect(nav.find(AnimalesMenu).length).toEqual(1);
	expect(nav.find(ContabilidadMenu).length).toEqual(1);
	expect(nav.find(InventariosMenu).length).toEqual(1);
	expect(nav.find(ConfiguracionMenu).length).toEqual(1);
  ...
  it('Renders a Header component in prod profile with no logged in User', () => {
    ...
    expect(nav.find(AdminMenu).length).toEqual(0);
    expect(nav.find(FincaMenu).length).toEqual(0);
	expect(nav.find(AnimalesMenu).length).toEqual(0);
	expect(nav.find(ContabilidadMenu).length).toEqual(0);
	expect(nav.find(InventariosMenu).length).toEqual(0);
	expect(nav.find(ConfiguracionMenu).length).toEqual(0);
    const account = nav.find(AccountMenu);
