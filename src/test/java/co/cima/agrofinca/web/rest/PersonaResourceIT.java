...
class PersonaResourceIT {
  ...
  void createPersona() throws Exception {
    ...
    restPersonaMockMvc
      .perform(
        post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(persona))
      )
  ...
  void putWithMissingIdPathParamPersona() throws Exception {
    ...
    // If url ID doesn't match entity ID, it will throw BadRequestAlertException
    restPersonaMockMvc
      .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(persona)))
  ...
  void patchWithMissingIdPathParamPersona() throws Exception {
    ...
    restPersonaMockMvc
      .perform(
        patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(persona))
      )
      .andExpect(status().isMethodNotAllowed());
